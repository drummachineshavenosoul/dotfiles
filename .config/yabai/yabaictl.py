...
setups = {
    "home": [
        "C80C2C26-53FE-EF76-5F57-62659E82650E",
...
class WindowManager:
    ...
    cache_path = expanduser("~/.cache/yabaictl")
    ...
    def move_window_to_space(self, window, space):
        yabai_message("window", str(window), "--space",
                      self._space_label(space))
    ...
    def remove_unnecessary_spaces(self):
        if self.num_spaces > self.NUM_SPACES:
            for unlabled_space in self.unlabled_spaces:
                yabai_message(
                    "space", f"{unlabled_space['index']}", "--destroy")
    ...
    def ensure_labels(self):
        wanted_labels = set(self._space_label(i)
